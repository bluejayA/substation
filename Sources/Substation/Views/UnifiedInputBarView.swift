...
struct UnifiedInputBarView {
    ...
    private static func getFilteredResultCount(tui: TUI, query: String) -> Int? {
        ...
        case .servers:
            count = FilterUtils.filterServers(tui.cachedServers, query: query).count
        case .serverGroups:
            count = FilterUtils.filterServerGroups(tui.cachedServerGroups, query: query).count
        case .networks:
            count = FilterUtils.filterNetworks(tui.cachedNetworks, query: query).count
        case .securityGroups:
            count = FilterUtils.filterSecurityGroups(tui.cachedSecurityGroups, query: query).count
        case .volumes:
            count = FilterUtils.filterVolumes(tui.cachedVolumes, query: query).count
        case .images:
            count = FilterUtils.filterImages(tui.cachedImages, query: query).count
        case .flavors:
            count = FilterUtils.filterFlavors(tui.cachedFlavors, query: query).count
        case .subnets:
            count = FilterUtils.filterSubnets(tui.cachedSubnets, query: query).count
        case .ports:
            count = FilterUtils.filterPorts(tui.cachedPorts, query: query).count
        case .routers:
            count = FilterUtils.filterRouters(tui.cachedRouters, query: query).count
        case .floatingIPs:
            count = FilterUtils.filterFloatingIPs(tui.cachedFloatingIPs, query: query).count
        case .keyPairs:
            count = FilterUtils.filterKeyPairs(tui.cachedKeyPairs, query: query).count
        default:
            return nil
        }
