...
// MARK: - Volume Server Management Input Handler

@MainActor
extension TUI {

    internal func handleVolumeServerManagementInput(_ ch: Int32, screen: OpaquePointer?) async {
        guard viewCoordinator.currentView == .volumeServerManagement else { return }
        ...
        let _ = await formInputHandler.handleManagementInput(
            ...
            onEnter: {
                ...
                if let module = ModuleRegistry.shared.module(for: "volumes") as? VolumesModule {
                    await module.performEnhancedVolumeManagement()
                }
            },
