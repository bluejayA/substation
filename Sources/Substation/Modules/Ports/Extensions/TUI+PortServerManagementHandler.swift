...
extension TUI {
    ...
    internal func handlePortServerManagementInput(_ ch: Int32, screen: OpaquePointer?) async -> Bool {
        ...
        return await formInputHandler.handleManagementInput(
            ...
            onEnter: {
                guard self.selectionManager.selectedServerId != nil else {
                    self.statusMessage = "Please select a server first"
                    return
                }
                guard let port = self.viewCoordinator.selectedResource as? Port else {
