...
struct AdvancedSearchView {
            ...
            case .cancelled:
                if showingResultDetail || showingCrossServiceResults {
                    ...
                    if let tuiInstance = tui {
                        tuiInstance.changeView(to: tuiInstance.viewCoordinator.previousView)
                        return true
                    }
                }
                return false
    ...
    private static func navigateToDetailView() {
        ...
        if let detailView = mapResourceToDetailView(selectedResult.resourceType) {
            ...
            Logger.shared.logNavigation(".advancedSearch", to: "\(detailView)", details: [
                ...
                "selectedIndex": "\(tuiInstance.viewCoordinator.selectedIndex)"
            ])
    ...
    private static func selectResourceInCache(_ tui: TUI, _ result: SearchResult) -> Bool {
        ...
        case .server:
            if let index = tui.cacheManager.cachedServers.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .network:
            if let index = tui.cacheManager.cachedNetworks.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .subnet:
            if let index = tui.cacheManager.cachedSubnets.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .port:
            if let index = tui.cacheManager.cachedPorts.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .router:
            if let index = tui.cacheManager.cachedRouters.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .volume:
            if let index = tui.cacheManager.cachedVolumes.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .image:
            if let index = tui.cacheManager.cachedImages.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .flavor:
            if let index = tui.cacheManager.cachedFlavors.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .securityGroup:
            if let index = tui.cacheManager.cachedSecurityGroups.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .keyPair:
            if let index = tui.cacheManager.cachedKeyPairs.firstIndex(where: { $0.name == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .floatingIP:
            if let index = tui.cacheManager.cachedFloatingIPs.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .serverGroup:
            if let index = tui.cacheManager.cachedServerGroups.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .volumeSnapshot:
            if let index = tui.cacheManager.cachedVolumeSnapshots.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .volumeBackup:
            if let index = tui.cacheManager.cachedVolumeBackups.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .barbicanSecret:
            if let index = tui.cacheManager.cachedSecrets.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .loadBalancer:
            if let index = tui.cacheManager.cachedLoadBalancers.firstIndex(where: { $0.id == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .swiftContainer:
            if let index = tui.cacheManager.cachedSwiftContainers.firstIndex(where: { $0.name == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        case .swiftObject:
            if let objects = tui.cacheManager.cachedSwiftObjects, let index = objects.firstIndex(where: { $0.name == resourceId }) {
                tui.viewCoordinator.selectedIndex = index
                return true
            }
        }
    ...
    private static func navigateToResourceDetail(_ result: SearchResult, tui: TUI) async {
        ...
        switch result.resourceType {
        case .server:
            if let server = tui.cacheManager.cachedServers.first(where: { $0.id == result.resourceId }) {
                tui.viewCoordinator.selectedResource = server
        ...
        case .network:
            if let network = tui.cacheManager.cachedNetworks.first(where: { $0.id == result.resourceId }) {
                tui.viewCoordinator.selectedResource = network
                tui.changeView(to: .networkDetail, resetSelection: false)
            } else {
                tui.changeView(to: .networks)
            }
        case .volume:
            if let volume = tui.cacheManager.cachedVolumes.first(where: { $0.id == result.resourceId }) {
        ...
        case .image:
            if let image = tui.cacheManager.cachedImages.first(where: { $0.id == result.resourceId }) {
                tui.viewCoordinator.selectedResource = image
                tui.changeView(to: .imageDetail, resetSelection: false)
            } else {
                tui.changeView(to: .images)
            }
        case .flavor:
            if let flavor = tui.cacheManager.cachedFlavors.first(where: { $0.id == result.resourceId }) {
                tui.viewCoordinator.selectedResource = flavor
                tui.changeView(to: .flavorDetail, resetSelection: false)
            } else {
                tui.changeView(to: .flavors)
            }
        case .port:
            if let port = tui.cacheManager.cachedPorts.first(where: { $0.id == result.resourceId }) {
        ...
        case .floatingIP:
            if let floatingIP = tui.cacheManager.cachedFloatingIPs.first(where: { $0.id == result.resourceId }) {
        ...
        case .router:
            if let router = tui.cacheManager.cachedRouters.first(where: { $0.id == result.resourceId }) {
    ...
    private static func createLiveSearchResults(from tui: TUI) async -> [SearchResult] {
        ...
        for volume in tui.cacheManager.cachedVolumes {
            results.append(SearchResult(
        ...
        for image in tui.cacheManager.cachedImages {
            results.append(SearchResult(
                resourceId: image.id,
                resourceType: .image,
                name: image.name,
        ...
        for flavor in tui.cacheManager.cachedFlavors {
            results.append(SearchResult(
    ...
    private static func createSearchableResources(from tui: TUI) -> SearchableResources {
        return SearchableResources(
            ...
            routers: tui.cacheManager.cachedRouters,
            volumes: tui.cacheManager.cachedVolumes,
            images: tui.cacheManager.cachedImages,
            flavors: tui.cacheManager.cachedFlavors,
            securityGroups: tui.cacheManager.cachedSecurityGroups,
            keyPairs: tui.cacheManager.cachedKeyPairs,
            floatingIPs: tui.cacheManager.cachedFloatingIPs,
            serverGroups: tui.cacheManager.cachedServerGroups,
            volumeSnapshots: tui.cacheManager.cachedVolumeSnapshots,
            volumeBackups: tui.cacheManager.cachedVolumeBackups,
            barbicanSecrets: tui.cacheManager.cachedSecrets,
            loadBalancers: tui.cacheManager.cachedLoadBalancers,
            swiftContainers: tui.cacheManager.cachedSwiftContainers,
            swiftObjects: tui.cacheManager.cachedSwiftObjects ?? []
        )
    }
