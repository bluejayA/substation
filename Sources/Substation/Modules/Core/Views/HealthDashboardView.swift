...
"""
    ...
    private static func addCacheMetrics(
        ...
        guard let cache = tui.cacheManager else { return }
        let serverCount = cache.cachedServers.count
        let networkCount = cache.cachedNetworks.count
        let volumeCount = cache.cachedVolumes.count
        let imageCount = cache.cachedImages.count
        let flavorCount = cache.cachedFlavors.count
        let floatingIPCount = cache.cachedFloatingIPs.count
        let securityGroupCount = cache.cachedSecurityGroups.count
        let routerCount = cache.cachedRouters.count

        let totalCachedResources = serverCount + networkCount + volumeCount + imageCount + flavorCount + floatingIPCount + securityGroupCount + routerCount
        ...
        let computeCount = serverCount + flavorCount
        let netCount = networkCount + floatingIPCount + routerCount + securityGroupCount
        let storageCount = volumeCount + imageCount
