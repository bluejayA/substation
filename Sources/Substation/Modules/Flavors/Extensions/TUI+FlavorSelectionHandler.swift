...
extension TUI {
    ...
    var flavorSelectionNavigationContext: NavigationContext {
        ...
        case .manual:
            ...
            if let query = searchQuery, !query.isEmpty {
                filteredFlavors = cacheManager.cachedFlavors.filter {
                    ($0.name?.localizedCaseInsensitiveContains(query) ?? false)
                }.sorted { ($0.name ?? "").localizedCaseInsensitiveCompare($1.name ?? "") == .orderedAscending }
            } else {
                filteredFlavors = cacheManager.cachedFlavors.sorted { ($0.name ?? "").localizedCaseInsensitiveCompare($1.name ?? "") == .orderedAscending }
            }
        ...
        case .workloadBased:
            if serverCreateForm.selectedCategoryIndex != nil {
                ...
                let categories = WorkloadCategoryHelpers.generateFilteredWorkloadTypes(from: cacheManager.cachedFlavors)
    ...
    internal func handleFlavorSelectionInput(_ ch: Int32, screen: OpaquePointer?) async {
        guard viewCoordinator.currentView == .flavorSelection else { return }
    ...
    private func handleFlavorSelectionSpace(screen: OpaquePointer?) async {
        ...
        case .manual:
            ...
            if viewCoordinator.selectedIndex < filteredFlavors.count {
                let selectedFlavor = filteredFlavors[viewCoordinator.selectedIndex]
                serverCreateForm.selectedFlavorID = selectedFlavor.id
            }
        case .workloadBased:
            if serverCreateForm.selectedCategoryIndex == nil {
                // In category list view - drill into the selected category
                serverCreateForm.selectedCategoryIndex = viewCoordinator.selectedIndex
                viewCoordinator.selectedIndex = 0
                viewCoordinator.scrollOffset = 0
                // Clear only the main panel area to remove artifacts before redrawing
                if let screen = screen {
                ...
                if let categoryIdx = serverCreateForm.selectedCategoryIndex, categoryIdx < categories.count {
                    ...
                    let sortedFlavors = WorkloadCategoryHelpers.selectTopFlavorsForWorkload(cacheManager.cachedFlavors, workloadType: workloadType, count: 3)

                    if viewCoordinator.selectedIndex < sortedFlavors.count {
                        let selectedFlavor = sortedFlavors[viewCoordinator.selectedIndex]
                        // Toggle selection
